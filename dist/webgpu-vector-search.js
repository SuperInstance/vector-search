 ...
 */
import { cosineSimilarity as cpuCosineSimilarity, normalizeVector, DEFAULT_EMBEDDING_DIM, } from './vector-utils';
...
export class WebGPUVectorSearch {
    constructor(dimension = DEFAULT_EMBEDDING_DIM, options) {
        this.device = null;
    ...
    async initializeGPU() {
        ...
        try {
            ...
            throw new WebGPUInitializationError('Failed to initialize WebGPU device', error);
        }
    }
    /**
     ...
     */
    async batchSearch(queries, vectors, k) {
        const numVectors = vectors.length / this.dimension;
        let gpuTime = 0;
        let cpuTime = 0;
        let usedGPU = false;
    ...
    // GPU IMPLEMENTATION
    // ==========================================================================
    async searchGPU(query, vectors, k) {
        if (!this.device) {
        ...
        commandEncoder.copyBufferToBuffer(resultsBuffer, 0, stagingBuffer, 0, numVectors * 4);
        // Submit commands
    ...
    async batchSearchGPU(queries, vectors, k) {
        // Process queries in batches
        const results = [];
        for (let i = 0; i < queries.length; i += this.batchSize) {
            const batch = queries.slice(i, i + this.batchSize);
            const batchResults = await Promise.all(batch.map(query => this.searchGPU(query, vectors, k)));
     ...
     */
    calculateOptimalBatchSize(dimension) {
