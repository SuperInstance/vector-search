...
import { cosineSimilarity, estimateTokens as estimateTokensUtil, hashEmbedding, DEFAULT_EMBEDDING_DIM, MAX_EMBEDDING_CACHE_SIZE, MIN_KEYWORD_LENGTH, KEYWORD_MATCH_BOOST, DEFAULT_SIMILARITY_THRESHOLD, DEFAULT_SEARCH_LIMIT, } from './vector-utils';
// ============================================================================
// VECTOR STORE
// ============================================================================
...
export class VectorStore {
    constructor(options) {
        ...
        this.maxCacheSize = MAX_EMBEDDING_CACHE_SIZE;
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                if (filter?.starred !== undefined) {
                    results = results.filter((e) => e.metadata.starred === filter.starred);
                }
                // Sort by timestamp
    ...
    async search(query, options = {}) {
        ...
        await this.ensureInitialized();
        const { limit = DEFAULT_SEARCH_LIMIT, threshold = DEFAULT_SIMILARITY_THRESHOLD, types, dateRange, tags, starredOnly = false, } = options;
     ...
     * ```
     */
    async setCheckpointStarred(checkpointId, starred) {
    ...
    async rollbackToCheckpoint(checkpointId) {
        ...
        const toRemove = currentEntries.filter(e => new Date(e.metadata.timestamp).getTime() > checkpointTime);
        for (const entry of toRemove) {
            await this.deleteEntry(entry.id);
        }
        // Restore any edited entries to their original state
        const entriesToRestore = currentEntries.filter(e => e.editedContent &&
     ...
     * ```
     */
    async getLatestStableCheckpoint() {
     ...
     * ```
     */
    async exportForLoRA(checkpointId, format = 'jsonl') {
        ...
        const entries = await this.getEntries();
        const validEntries = entries.filter(e => new Date(e.metadata.timestamp).getTime() <= checkpointTime);
        // Calculate statistics
        const totalTokens = validEntries.reduce((sum, e) => sum + this.estimateTokens(e.content), 0);
        const avgQuality = validEntries.reduce((sum, e) => sum + (e.metadata.importance || 0.5), 0) / validEntries.length;
